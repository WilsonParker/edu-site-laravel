...
@section('content')
    <div id="section" class="page-qna">
        ...
        <div class="sub_content">
            <form name="counsel" method="post" action="{{ route('qna.update', ['qna'=>$data->idx]) }}"
                  onSubmit="return Submit_Counsel();"
                ...
                <table class="t_style33">
                    ...
                    <tr>
                        <th>제목</th>
                        <td><input type="text" name="title" maxlength="100" value="{{ $data->title }}" required/>
                        </td>
                    </tr>
                    <tr>
                        <th>파일첨부</th>
                        <td>
                            @isset($data->resource)
                                <p id="originfile" class="file-text">
                                    {{ $data->resource->origin_name }}
                                    <button type='button' onclick='qnaRemoveFile()' class='file-remove'></button>
                                </p>
                                <div id="previews"></div>
                            @else
                    ...
                    <tr>
                        <th>내용</th>
                        <td><textarea name="contents" required>{{ $data->contents }}</textarea></td>
                    </tr>
                </table>
...
@push('body_scripts')
    <script>
        ...
        function qnaRemoveFile() {
            const originfile = document.querySelector('#originfile');
            originfile.remove();
            const previews = document.querySelector('#previews');
            previews.innerHTML += `
                    <input type="hidden" name="fileDelete" value="ok"/>
					<input type="file" id="file-input" name="file" value="첨부파일" />
					<label for="file-input" class="file-submit">첨부하기</label>
                    <div id="preview"></div>`;
            handler.init()
